...
async function iniciarBanco(){
  try {
    ...
    if (existe.length === 0) {
      const hash = await bcrypt.hash('admin123', 12); // Salt aumentado para mais segurança
...
app.use(session({
  ...
  cookie: {
    ...
    httpOnly: true,
    sameSite: 'strict', // Mais restrito contra roubo de sessão
...
const armazenamento = multer.diskStorage({
  ...
  filename: (req, arq, cb) => cb(null, Date.now() + '-' + arq.originalname.replace(/\s+/g, '_').replace(/[^a-zA-Z0-9._-]/g, ''))
});
const upload = multer({ 
  ...
  fileFilter: (req, file, cb) => {
    ...
    const extPermitida = tiposPermitidos.test(path.extname(file.originalname).toLowerCase());
    const mimeTypePermitido = tiposPermitidos.test(file.mimetype);
    if (extPermitida && mimeTypePermitido) return cb(null, true);
...
app.post('/logar', async (req, res) => {
  try {
    ...
    if (!login || !senha) return res.render('login', { erro: 'Preencha todos os campos!' });
    ...
    res.render('login', { erro: 'Erro interno, tente novamente mais tarde' });
  }
});
...
app.post('/cadastrar-usuario', async (req, res) => {
  try {
    ...
    if (!login || !senha) return res.render('cadastrar-usuario', { erro: 'Preencha login e senha!', sucesso: null, usuarioLogado: req.session.usuario || null });
...
app.get('/editar/:id', soLogado, async (req, res) => {
  try {
    ...
    if (!p[0]) return res.redirect('/busca');
...
app.post('/salvar', soLogado, upload.array('fotos', 10), async (req, res) => {
  ...
  try {
    ...
    if (dados.id) {
      ...
      const [result] = await conn.execute(`INSERT INTO pessoas (siapen,nome,cpf,rg,nascimento,mae,pai,cep,rua,numero,bairro,cidade,uf,complemento,processo_unificado,pena_total,data_progressao,fotos) VALUES (?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?)`,
...
app.post('/excluir/:id', soAdmin, async (req, res) => {
  try {
...
// ==============================================
// 🛠️ ROTAS TEMPORÁRIAS (USAR 1 VEZ APENAS)
// ==============================================
